...
echo"<div id='changeinfo'>
    <form id=info action='ProviderPage.php' method='get'>
        <table class='tg'>";
            ...
            echo" </table>
        <input id=status onclick='update()'  class=update value='Edit' readonly='readonly'/>
    </form>
</div>";
...
<div id="right">
    <div id='inner'>
        ...
        echo"<div id='menulist'><form id='pform' action='ProviderPage.php' method='get'>
            <dl id='dlstyle'>";
        $sqlOrder="SELECT DISTINCT ORDERID,CUSTID,STATUS,ORDERTIME,DELIVERYTIME,DELIVERYMETHOD,TO_CHAR(ORDERTIME, 'MON-DD-YYYY HH24:MI:SS') TIME_STR
                   FROM G14ORDER
                   WHERE BUSSID='$userID'
        ...
        echo "<dd id=LM" .$j . ($nrows) . " style='DISPLAY: none'><div id='item'>
                 ...
                 <span id=i3>&nbsp;Dly.Time:&nbsp<input id='dly' type='text' name='dly[]' readonly='readonly' value='".$DliveryTime."'/>min</span></div></dd>\n";
        ...
        echo"</dl><input id='porder' onclick='pupdate()' class=update  value='Update' style='margin-left:35%' readonly='readonly'/></form></div>";
        ?>
...
<script>
    ...
    function pupdate() {
        if(document.getElementById("porder").value=="Update"){
            ...
            document.getElementById("porder").value="Confirm";
            document.getElementById("porder").style.width="206px";
            document.getElementById("porder").style.height="36px";
            document.getElementById("porder").type="submit";
        } else
        if(document.getElementById("porder").value=="Confirm"){
            var form2 = document.getElementById("pform");
            var elements2 = form2.elements;
            for (var j = 0, lengthx = elements2.length; i < lengthx-1; ++j) {
                elements2[i].readOnly = true;
                elements2[i].style.borderBottom="1px solid transparent";
            }
            document.getElementById("porder").value="Update";
            document.getElementById("porder").style.width="206px";
            document.getElementById("porder").style.height="36px";
            document.getElementById("porder").type="submit";
        }
